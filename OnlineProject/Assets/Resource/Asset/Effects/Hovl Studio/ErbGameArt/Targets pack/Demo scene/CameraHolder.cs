...
public class CameraHolder : MonoBehaviour
{
    ...
    void Start ()
    {
        ...
        screenGUI.normal.textColor = new Color(0f, 0f, 0f);
    ...
    void CurrPrefabNomber(int delta)
    {
        currentPrefab += delta;
        if (currentPrefab > Prefabs.Length - 1)
        {
            currentPrefab = 0;
        }
        ...
        if (currentPrefabInstance != null) Destroy(currentPrefabInstance);
        {
            currentPrefabInstance = Instantiate(Prefabs[currentPrefab]);
        }
    }
